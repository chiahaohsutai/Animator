 ...
 * The shape being visited should always have a valid name.
 */
public class SVGShapeVisitor implements ISVisitor {
  ...
  public void visitRect(Rect rect) {
    ...
    this.results = String.format("<rect id=\"%s\" x=\"%.3f\" y=\"%.3f\" width=\"%.3f\" " +
            "height=\"%.3f\" fill=\"rgb(%d,%d,%d)\" visibility=\"hidden\"> </rect>",
  ...
  public void visitEllipse(Ellipse ellipse) {
    ...
    if (ellipse.getWidth() == ellipse.getHeight()) {
      this.results = String.format("<circle id=\"%s\" cx=\"%.3f\" cy=\"%.3f\" r=\"%.3f\" " +
              "fill=\"rgb(%d,%d,%d)\" visibility=\"hidden\"> </circle>", ellipse.getName(),
              ellipse.getX(), ellipse.getY(), ellipse.getHeight(), ellipse.getRed(),
              ellipse.getGreen(), ellipse.getBlue());
    } else {
      this.results = String.format("<ellipse id=\"%s\" cx=\"%.3f\" cy=\"%.3f\" rx=\"%.3f\" " +
                      "ry=\"%.3f\" fill=\"rgb(%d,%d,%d)\" visibility=\"hidden\"> </ellipse>", ellipse.getName(),
